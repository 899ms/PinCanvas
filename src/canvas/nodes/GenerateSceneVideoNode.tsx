...
import type {
  GenerateSceneVideoNode as GenSceneVidNodeT,
  NodeId,
} from '@/types/node';
import {
...
export function GenerateSceneVideoNodeComp({ id, selected }: NodeProps) {
  const nid = id as NodeId;
  const VIDEO_MODELS = useModels('video');
  const node = useCanvas(
    (s) => s.nodes.find((n) => n.id === nid) as GenSceneVidNodeT | undefined,
  );
  ...
  const scene = useLibrary((s) =>
    node?.settings.sceneId
      ? s.scenes.find((c) => c.id === node.settings.sceneId)
      : undefined,
  );
  ...
  const currentSettings =
    node?.kind === 'generate-scene-video' ? node.settings : undefined;
  ...
  useEffect(() => {
    if (!node || node.kind !== 'generate-scene-video' || isBusy) return;
    ...
    if (
      ...
      patchSettings<'generate-scene-video'>(nid, {
  ...
  if (!node || node.kind !== 'generate-scene-video') return null;
    ...
    <div className={frameClass(selected)}>
      ...
      <div className={`${NODE_BODY} flex flex-col gap-1.5 overflow-y-auto px-2 py-1.5`}>
        ...
        <select
          ...
          onChange={(e) =>
            patchSettings<'generate-scene-video'>(nid, { model: e.target.value })
          }
          disabled={isBusy}
        >
          {VIDEO_MODELS.map((m) => (
        ...
        <textarea
          ...
          placeholder="附加 video prompt（可选）"
          value={settings.videoPrompt ?? ''}
          onChange={(e) =>
            patchSettings<'generate-scene-video'>(nid, { videoPrompt: e.target.value })
          }
          disabled={isBusy}
        />
        <div className="flex gap-1">
          <select
            ...
            onChange={(e) =>
              patchSettings<'generate-scene-video'>(nid, { duration: e.target.value })
            }
            disabled={isBusy}
          >
          ...
          <select
            ...
            onChange={(e) =>
              patchSettings<'generate-scene-video'>(nid, { ratio: e.target.value })
            }
            disabled={isBusy}
          >
            {ratioOptions.map((r) => (
          ...
          <select
            ...
            onChange={(e) =>
              patchSettings<'generate-scene-video'>(nid, { resolution: e.target.value })
            }
            disabled={isBusy}
          >
        ...
        {scene?.imageUrl && (
          <div className="flex items-center gap-1">
            ...
            />
            <span className="text-[10px] text-zinc-400">首帧来自场景概念图</span>
          </div>
        )}
